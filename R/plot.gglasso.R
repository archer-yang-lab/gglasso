plot.gglasso <- function(x, alpha=1, type=c("indiv","group"), legend.loc, log.l=FALSE, ...)
  {
	type=match.arg(type)
	xb = x$beta
	if(nrow(xb)==1){
    ...
    plot.args <- list(x=l, y=1:length(l), ylim=range(beta), xlab=xlab, ylab="Coefficients", type="n", xlim=range(l))
    new.args <- list(...)
    if (length(new.args))
      {
    ...
    line.args <- list(col=gray.colors(n.g+1,start = 0.05, end = 0.7, gamma = 2.2)[1:n.g],lwd=1+1.2^(-p/20),lty=1)
	...
	line.args$col <- gray.colors(n.g+1,start = 0.05, end = 0.7, gamma = 2.2)
