cv.gglasso = function(x, y, group, lambda = NULL, 
	pred.loss = c("misclass", "loss", "L1", "L2"), 
	nfolds = 5, foldid, delta = 2, ...) {
    ...
    for (i in seq(nfolds)) {
        which = foldid == i
        y_sub = y[!which]
        outlist[[i]] = gglasso(x = x[!which, , drop = FALSE], y = y_sub, 
...
cv.hsvm = function(outlist, lambda, x, y, foldid, 
    pred.loss, delta) {
...
cv.sqsvm = function(outlist, lambda, x, y, foldid, 
    pred.loss, delta) {
    typenames = c(misclass = "Misclassification Error", loss = "Margin Based Loss")
    if (pred.loss == "default")
        pred.loss = "misclass"
